<?
/* $Id: dns.inc.php 66 2012-08-26 20:01:51Z taira $ */
...
class DNS {
    ...
    function addZone($domain, $address, $password) {
        if($this->isDomain($domain) && $this->isIP($address)) {
            if($this->isZone($domain)) {
                $this->raiseError("ドメインが既に存在しているか、キューに存在している状態です\n");
                return false;
            }
    ...
    function createLock($process) {
        if($this->isLock($process)) {
            $this->raiseError("プロセスが既に起動しています\n");
            return false;
        }
        else {
    ...
    function reloadZone($domain = "") {
        ...
        $zonefile = $this->path."/".$domain;
        if (empty($domain)){
           exec("/opt/named/sbin/named-checkzone $domain $zonefile", $exec, $result);
        if ($result === 0 ) {
              exec("/opt/named/sbin/rndc reload $domain", $exec);
           } else {
              return false;
           } 
        }
    ...
    function delZone($domain) {
        if($this->isDomain($domain)) {
            ...
            $this->db->Execute("DELETE FROM records_txt WHERE domainid = $domainid");
...
//        }
        ...
        //txt records
        $query = $this->db->Execute("SELECT name, txtvalue FROM records_txt WHERE domainid = '$domainid'");
        if($query->numRows() > 0) {
            $find = array("{name}", "{txtvalue}");
            while($row = $query->fetchRow()) {
                $replace = array($row['name'], $row['txtvalue']);
                $records_cname.= str_replace($find, $replace, $this->tpl['record_txt']);
            }
        }


        $zone .= $records_mx . $records_a . $records_cname . $records_txt;
        ...
        if($return) {
            ...
            fwrite($f, $zone);
            $src_file .= $this->path . "/$letter/" . $domain;
            $dst_file .= $this->path . "/" . $domain; 
            exec("/bin/cp -p $src_file $dst_file");
            fclose($f);
        }
    }
    ...
    function loadTpl() {
        ...
        $this->tpl['record_txt'] = file_get_contents($this->tpl_path . "/record_txt.tpl");
    ...
    function recordInfo($domain, $recordid, $type) {
        ...
        switch(strtolower($type)) {
            ...
            break;
            case 'txt':
                $sql = "SELECT name, txtvalue AS address FROM records_txt WHERE domainid = $domainid AND recordid=$recordid";
            break;
            default:
                $sql = false;
    ...
    function delRecord($domain, $recordid, $type) {
        ...
        switch(strtolower($type)) {
            ...
            break;
            case 'txt':
                $sql = "DELETE FROM records_txt WHERE recordid = $recordid AND domainid = $domainid";
            break;
            default:
                $sql = false;
                $this->raiseError("Invalid Entry\n");
        ...
        switch(strtolower($type)) {
            ...
            case 'txt':
                if(!preg_match("/^\".*\"$/", $address['address'])) {
                    $this->raiseError("TXTはダブルクォートで囲んで下さい\n");
                    return false;
                }
                $sql = "UPDATE records_txt set txtvalue = $maddress WHERE recordid = $recordid AND domainid = $domainid";
            break;
            case 'mx':
                $ip = $address['address'];
                $name = $address['name'];
                if(!preg_match("/.*\.$/", $ip)) {
                    $this->raiseError("MXレコードの値の最後はピリオドを付けて下さい\n");
                    return false;
                }
            ...
            switch(strtolower($type)) {
                ...
                case 'txt':
                    if(!preg_match("/.*\"$/", $address)) {
                        $this->raiseError("TXTレコードはダブルクォートで囲んで下さい\n");
                        return false;
                    }
			//ここにTXTレコードのバリデーションのモジュール記載
                    $query = $this->db->Execute("SELECT recordid FROM records_txt WHERE domainid = '$domainid' AND txtvalue = $safename LIMIT 1");
                    if($query->numRows() > 0) {
                        $this->raiseError("Record Exists\n");
                        return false;
                    }
                    $this->db->Execute("INSERT INTO records_txt (domainid, name, txtvalue) VALUES ('$domainid', $safename, $safeaddress)");
                break;
                case 'mx':
                    if(!preg_match("/.*\.$/", $address)) {
                        $this->raiseError("MXレコードの値の最後はピリオドを付けて下さい\n");
    ...
    function getRecords($domain, $type) {
        ...
        switch(strtolower($type)) {
            ...
            break;
            case 'txt':
                $records = $this->db->getAll("SELECT recordid, name, txtvalue FROM records_txt WHERE domainid = $domainid ORDER BY name");
            break;
            case 'mx':
